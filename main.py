...
TOKEN = "1648917738:AAG4daDffNF37NxPnkPLqVH06d1H4JQ_Tks"

router = Router()

keyboard = ReplyKeyboardBuilder()
keyboard.add(KeyboardButton(text="Улыўма толемди билиў"), KeyboardButton(text="Улыума қарзды билиў"),
             KeyboardButton(text="Толеўлер тарийхи"), KeyboardButton(text="Толеў графигы"))
keyboard.adjust(1)


@router.message(Command("start"))
async def command_start_handler(message: Message) -> None:
    await message.answer(f"Ассалаума алейкум, {hbold(message.from_user.full_name)} \nКерекли болимди таңлаң",
                         reply_markup=keyboard.as_markup(resize_keyboard=True)
                         )


@router.message(F.text == "Улыўма толемди билиў")
...
@router.message(F.text == "Улыума қарзды билиў")
async def with_puree(message: types.Message):
    duty = get_duty()
...
@router.message(F.text == "Толеў графигы")
async def with_puree(message: types.Message):
...
@router.message(F.text == "Толеўлер тарийхи")
async def with_puree(message: types.Message):
